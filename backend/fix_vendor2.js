...
mongoose.connect(process.env.MONGO_URI)
    .then(async () => {
        ...
        const customer = await User.findOne({ email: 'darrangbhai@gmail.com' });
        // Find the vendor account that was created without email
        const vendor = await User.findOne({ phone: '6001275419', role: 'vendor' });

        if (customer && vendor) {
            // Update the vendor with the email and google details
            vendor.email = 'darrangbhai@gmail.com';
