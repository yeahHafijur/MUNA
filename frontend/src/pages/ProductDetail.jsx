...
import { useAuth } from '../context/AuthContext'; // Added for wishlist sync
...
/* ─── Optimization 5: Global Haptic Helper ─── */
...
const ProductDetail = () => {
    ...
    /* ─── Optimization 3: Wishlist Backend Sync ─── */
    const toggleLike = async () => {
        triggerHaptic(30);
        const newLiked = !liked;
        setLiked(newLiked);

        // Instant Optimistic UI Update via localStorage
        const savedLikes = JSON.parse(localStorage.getItem('muna_likes') || '{}');
        ...
        // Background API Sync (Only if user is logged in)
        if (token) {
            try {
                ...
                console.error('Wishlist sync failed in background', err);
            }
        }
    };
    ...
    /* ─── Optimization 1: Fetch Specific Product Only ─── */
    ...
    /* ─── Optimization 4: Robust Cart Error Handling ─── */
    const handleAdd = () => {
    ...
    return (
        <div className="min-h-screen bg-white font-sans pb-28 animate-in fade-in duration-300">
            ...
            <div className="p-5 max-w-3xl mx-auto">
                ...
                {/* ─── Optimization 2: Dynamic Content Rendering ─── */}
                <div>
                    ...
                    <p className="text-[14px] text-slate-600 leading-relaxed font-medium whitespace-pre-line">
                        {product.description || `This is a premium quality product sourced directly from ${shop.name}. We take pride in ensuring that you receive the freshest and best items delivered to your doorstep swiftly.`}
                    </p>
                </div>
            </div>
