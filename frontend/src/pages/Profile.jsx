...
const IconPackage = () => <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M21 7.5l-9-5.25L3 7.5m18 0l-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" /></svg>;
...
const IconStore = () => <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H4.5m0 0V8.25M4.5 21h15M4.5 8.25L12 3l7.5 5.25M4.5 8.25h15M18.75 21H21v-7.5a.75.75 0 00-.75-.75h-3a.75.75 0 00-.75.75V21M12 12.75v.008" /></svg>;

/* ─── Unified Menu Row (no duplicate actions) ─── */
const MenuRow = ({ icon, title, subtitle, onClick, isDanger, isLast, badge }) => (
    <div
        onClick={() => {
            if (navigator.vibrate) navigator.vibrate(40);
            onClick();
        }}
        className={`flex items-center justify-between px-5 py-4 bg-white active:bg-slate-50 transition-colors cursor-pointer
      ${!isLast ? 'border-b border-slate-50' : ''}`}
    >
        <div className="flex items-center gap-4 min-w-0 flex-1">
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 ${isDanger ? 'bg-red-50 text-red-500' : 'bg-slate-50 text-slate-700'
                }`}>
                {icon}
            </div>
            <div className="flex flex-col min-w-0">
                <span className={`text-[15px] font-semibold tracking-tight truncate ${isDanger ? 'text-red-600' : 'text-slate-900'}`}>
                    {title}
                </span>
                {subtitle && <span className="text-xs font-medium text-slate-400 mt-0.5 truncate">{subtitle}</span>}
            </div>
        </div>
        <div className="flex items-center gap-2 ml-2">
            {badge && <span className="text-xs font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{badge}</span>}
...
const SectionHeader = ({ title }) => (
    <h3 className="text-xs font-bold uppercase tracking-[0.1em] text-slate-400 px-5 mb-2 mt-6 first:mt-0">{title}</h3>
);
...
const Profile = () => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex flex-col bg-[#F8FAFC] overflow-hidden font-sans"
            ...
            {/* ─── NATIVE HEADER ─── */}
            <div className="sticky top-0 z-20 backdrop-blur-xl bg-white/80 border-b border-white/40 shadow-sm">
                <div className="flex items-center justify-between px-5 py-3">
                    <button
                        onClick={() => navigate(-1)}
                        className="w-10 h-10 rounded-full bg-slate-100/80 flex items-center justify-center text-slate-700 active:scale-95 transition-transform"
                    >
                        <IconBack />
                    </button>
                    <span className="text-base font-bold text-slate-900 tracking-tight">Profile</span>
            ...
            <div className="flex-1 overflow-y-auto pb-24 scrollbar-hide">
                ...
                <div className="px-5 pt-6 pb-2">
                    <div className="bg-white rounded-[28px] p-5 shadow-[0_20px_40px_rgba(0,0,0,0.06)] border border-slate-100">
                        <div className="flex items-start gap-5">
                            ...
                            <div className="flex flex-col min-w-0 flex-1 pt-1">
                                <h2 className="text-xl font-bold text-slate-900 truncate tracking-tight">{user.name}</h2>
                                <p className="text-[13px] text-slate-500 font-medium truncate">{user.email}</p>
                                <div className="flex items-center gap-2 mt-3">
                                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-amber-50 border border-amber-100 text-[11px] font-semibold text-amber-700 tracking-wide">
                                        📞 {user.phone || 'Add Phone'}
                                    </span>
                                    <button
                                        onClick={() => navigate('/profile/settings')}
                                        className="text-xs font-bold text-slate-500 bg-slate-50 px-3 py-1 rounded-full active:bg-slate-100 transition-colors"
                                    >
                                        Edit
                                    </button>
                                </div>
                ...
                <div className="px-5">
                    ...
                    <div className="bg-white rounded-[28px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-amber-100 overflow-hidden">
                        <MenuRow
                            ...
                            subtitle="Open your shop on MUNA"
                    ...
                    <div className="bg-white rounded-[28px] shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-red-50 overflow-hidden mb-8">
                        <MenuRow
                            icon={<IconLogout />}
                            title="Sign Out"
                            isDanger
                    ...
                    <div className="text-center pb-8">
                        <p className="text-xs font-medium text-slate-400">MUNA v2.0 · Made with ❤️</p>
                    </div>
                </div>
            </div>
        </div>
