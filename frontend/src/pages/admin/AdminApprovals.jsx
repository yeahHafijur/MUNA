...
const AdminApprovals = () => {
    ...
    const pendingGodown = allGodownItems.filter(i => i.status === 'pending');
        ...
        <div className="fixed inset-0 z-[100] bg-[#F8FAFC] flex flex-col font-sans">
            ...
            <div className="bg-white px-4 py-3 border-b border-slate-100 flex items-center gap-3 sticky top-0 z-50 shadow-sm">
                ...
                {pendingGodown.length > 0 && (
                    <span className="ml-auto px-2.5 py-1 rounded-md bg-rose-500 text-white text-[10px] font-black leading-none">{pendingGodown.length}</span>
                )}
            </div>
                        ...
                        {pendingGodown.map(item => (
                            <div key={item._id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="text-sm font-black text-gray-900 truncate">{item.name}</div>
