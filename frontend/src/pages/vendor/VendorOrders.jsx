                ...
                {updatingStatusId === order._id ? (
                    ...
                    <>
                        ...
                        {(order.status === 'accepted' || order.status === 'preparing') && (
                            <>
                                <button className="v-btn v-btn-primary v-btn-full" onClick={() => handleStatus(order._id, 'out_for_delivery')}>Out for Delivery</button>
                                <button 
                                    onClick={() => handleWhatsAppShare(order)} 
                                    style={{ width: '100%', padding: '8px 12px', background: '#25D366', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontWeight: 600, marginTop: '4px' }}
                                >
                    ...
                    {liveOrders.length === 0 ? (
                        ...
                        <div className="v-kanban">
                            ...
                            <div className="v-kanban-col" style={{ borderTop: '3px solid var(--v-info)' }}>
                                <div className="v-kanban-header">
                                    <h3 style={{ color: 'var(--v-info)' }}>Accepted</h3>
                                    <span className="v-kanban-count">{acceptedOrders.length}</span>
                                </div>
                                <div className="v-kanban-body">
                                    {acceptedOrders.length === 0 ? <div className="v-kanban-empty">No accepted orders</div> : acceptedOrders.map(renderOrderCard)}
                                </div>
                            </div>
                            <div className="v-kanban-col" style={{ borderTop: '3px solid var(--v-success)' }}>
                                <div className="v-kanban-header">
                                    <h3 style={{ color: 'var(--v-success)' }}>Out for Delivery</h3>
